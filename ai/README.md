...
### 📈 Advanced Statistical Analytics & Forecasting
- **Traffic Peak Forecasting**: Predict traffic volumes and peak times using statistical baselines
- **Anomaly Detection**: Identify delays and no-shows using heuristic pattern recognition
- **Monthly Throughput Forecasting**: 1-month ahead predictions with EWMA trend analysis and saturation risk scoring
...
- **Proactive Alerts**: Operational warnings based on statistical thresholds
- **What-If Simulation**: Rule-based scenario analysis for capacity planning
...
- **Intelligent Intent Classification**: Google Gemini-powered intent detection via AGNO framework
- **Message Polishing**: Natural language response refinement for professional interactions
...
- **Performance Optimized**: Low-latency execution with strict deadlines
...
└── app/                                # Main source code
    ...
    │   ├── orchestrator.py             # Main orchestrator (Agno + Deterministic Fallback)
    │   ├── intent_detector.py          # Deterministic intent detection (Regex)
    ...
    │   ├── monthly_forecast_engine.py       # Forecasting engine (Time-series/EWMA)
    ...
    │   └── llm_provider.py             # Google Gemini integration via Agno
    │
...
# AGNO Intelligent Orchestration (Recommended)
AGNO_ENABLED=true
GOOGLE_AI_STUDIO_API_KEY=your-google-api-key-here  # Get from https://aistudio.google.com/apikey
LLM_MODEL_NAME=gemini-1.5-pro  # or gemini-1.5-flash for faster responses
LLM_TEMPERATURE=0.2
LLM_MAX_TOKENS=1024
...
# Install dependencies (AGNO, FastAPI, etc.)
...
- **Graceful Degradation**: Fallback to deterministic mode if AGNO/LLM is unavailable
...
- **Capacity Utilization Analysis**: Slot capacity vs throughput analysis
- **Monthly Forecasting**: 1-month ahead predictions with saturation risk (Statistical)
